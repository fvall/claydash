...
typedef struct {
    ...
    bool vertical;   // Clip overflowing elements on the YU axis and allow
...
typedef struct {
    ...
    Clay_CornerRadius cornerRadius;
    // The original dimensions of the source image, used to control aspect
    // ratio.
    ...
    for (int32_t j = 0; j < layoutElement->elementConfigs.length; j++) {
        ...
        if (config->type == CLAY__ELEMENT_CONFIG_TYPE_IMAGE) {
            Clay_ImageElementConfig *imageConfig =
                config->config.imageElementConfig;
...
void Clay__CloseElement(void) {
    ...
    for (int32_t i = 0; i < openLayoutElement->elementConfigs.length; i++) {
        ...
        if (config->type == CLAY__ELEMENT_CONFIG_TYPE_SCROLL) {
            ...
            elementHasScrollVertical =
                config->config.scrollElementConfig->vertical;
    ...
    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
        ...
        for (int32_t i = 0;
            ...
            // Minimum size of child elements doesn't matter to scroll
            // containers as they can shrink and hide their contents
        ...
        if (hierarchicalParent) {
            ...
            if (declaration->floating.attachTo == CLAY_ATTACH_TO_PARENT) {
                ...
                floatingConfig.parentId =
                    Clay__HashString(CLAY_STRING("Clay__RootContainer"), 0, 0)
                        .id;
            }
        ...
        for (int32_t i = 0; i < bfsBuffer.length; ++i) {
            ...
            if (sizingAlongAxis) {
                ...
                if (sizeToDistribute < 0) {
                    // If the parent can scroll in the axis direction in this
                    // direction, don't compress children, just leave them alone
                    Clay_ScrollElementConfig *scrollElementConfig =
                        Clay__FindElementConfigWithType(
                ...
                for (int32_t childOffset = 0;
                    ...
                    if (!xAxis &&
                        Clay__ElementHasConfig(
                            childElement, CLAY__ELEMENT_CONFIG_TYPE_IMAGE)) {
                        continue; // Currently we don't support resizing aspect
                                  // ratio images on the Y axis because it would
                                  // break the ratio
                    }
        ...
        while (dfsBuffer.length > 0) {
            ...
            if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
                ...
                if (Clay__ElementHasConfig(currentElement,
                    ...
                    Clay_ScrollElementConfig *scrollConfig =
                        Clay__FindElementConfigWithType(
                            currentElement, CLAY__ELEMENT_CONFIG_TYPE_SCROLL)
                            .scrollElementConfig;
...
Clay__RenderDebugLayoutElementsList(int32_t initialRootsLength,
    ...
    if (context->pointerInfo.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME) {
        Clay_ElementId collapseButtonId = Clay__HashString(
            CLAY_STRING("Clay__DebugView_CollapseElement"), 0, 0);
        ...
        CLAY({.id = scrollId,
            ...
            CLAY({.layout = {.sizing = {CLAY_SIZING_GROW(0),
                ...
                Clay_ElementId panelContentsId = Clay__HashString(
                    CLAY_STRING("Clay__DebugViewPaneOuter"), 0, 0);
                // Element list
                ...
                for (int32_t elementConfigIndex = 0;
                        ...
                        CLAY({.id = CLAY_ID(
                              ...
                              .layout = {.padding = attributeConfigPadding,
                                         ...
                                         .layoutDirection =
                                             CLAY_TOP_TO_BOTTOM}}) {
                            // .sourceDimensions
                            CLAY_TEXT(CLAY_STRING("Source Dimensions"),
                                      infoTitleConfig);
                            CLAY({.id = CLAY_ID(
                                      "Clay__"
                                      "DebugViewElementInfoImageDimensions")}) {
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_SCROLL: {
                        ...
                        CLAY({.layout = {.padding = attributeConfigPadding,
                            ...
                            CLAY_TEXT(scrollConfig->vertical
                                          ? CLAY_STRING("true")
                                          : CLAY_STRING("false"),
                                      infoTextConfig);
                            // .horizontal
                            CLAY_TEXT(CLAY_STRING("Horizontal"),
                                      infoTitleConfig);
                            CLAY_TEXT(scrollConfig->horizontal
                                          ? CLAY_STRING("true")
                                          : CLAY_STRING("false"),
                                      infoTextConfig);
                        }
        ...
        if (scrollContainerData->elementId == id.id) {
            Clay_ScrollElementConfig *scrollElementConfig =
                Clay__FindElementConfigWithType(
                    ...
                    CLAY__ELEMENT_CONFIG_TYPE_SCROLL)
                    .scrollElementConfig;
            if (!scrollElementConfig) { // This can happen on the first frame
